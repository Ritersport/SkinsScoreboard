...
import com.ritesrport.skinsscoreboard.domain.GameResultState
...
import com.ritesrport.skinsscoreboard.domain.HoleInputState
...
class MainViewModel(private val repository: HoleInputRepository) : ViewModel() {
    private val _holeInputState = runBlocking {  MutableStateFlow(
        HoleInputState(
    ...
    private fun onCompletePlayerInput(strokesNumber: Int)
    {
        runBlocking {
            ...
            if (nextPlayer != null) {
                ...
                if (nextHole != null) {
                    _holeInputState.update { HoleInputState(holeData = nextHole, player = repository.getFirstPlayer()) }
                }
                else
                {
                    setWinner()
    ...
    private fun onNewGame()
    {
        runBlocking {
            ...
            _holeInputState.update { HoleInputState(holeData = repository.getFirstHole(),
                player = repository.getFirstPlayer()) }
